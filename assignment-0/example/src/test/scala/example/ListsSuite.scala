...
class ListsSuite extends FunSuite {
  ...
  test("max of list with single number") {
    assert(max(List(7)) === 7)
  }
  
  test("max of list with negtive numbers") {
    assert(max(List(-1,-2,-3)) === -1)
  }
  
  test("max of list with duplicated numbers 1") {
    assert(max(List(-1,-2,-1)) === -1)
  }
  
    test("max of list with duplicated numbers 2") {
